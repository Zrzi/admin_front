...
class="content">
        ...
        "
                  ...
                  <div v-if="showForm == 1">
                               <el-page-header :icon="ArrowLeft" content="详情" @back="goback"/>
...
export default {
  ...
  created() {
    this.coursenum=store.state.courseNum
    bus.on('collapse', msg => {
      this.collapse = msg;
    });
  ...
  methods: {
              ...
              for (var pro in _self.form) {
                for (var i = 0; i < _self.formList.length; i++) {
                  if (pro == _self.formList[i].prop) {
                    _self.formList[i].value = _self.form[pro];
                    if(i==0) {
                      _self.coursenum1 = _self.formList[i].value;
                    }
                    if (_self.formList[i].type == "select") {
                      if (
                              _self.form[pro + "List"] != undefined &&
    ...
    addItem() {
          ...
          if (title === _self.page[i].title) {
            ...
            if (_self.page[i].type == "table") {
              ...
              _self.count1=_self.count
                            _self.name='homework'
        var urlF = "/api/" + 'teach' + "/" + "homeworkInit";
              generalRequest(urlF, data).then((res) => {
                _self.tableList = res.data.data;
          ...
          if(value["courseNum"] == _self.coursenum1){
        console.log(key)
                console.log('tset')
                console.log(_self.count1)
            _self.homeworkList[_self.count1]=value;
                      console.log(value);
                      _self.count1++
                        console.log(_self.homeworkList);
          }
      ...
      _self.list=_self.homeworkList

    },
    homeworks(title){
      this.$store.commit("setRouterName", title);
      this.$emit("title", title);
      this.homeworks1(title);
    },
    attendences1(title){
      let _self = this;
      getUimsConfig().then((res) => {
     ...
     console.log(jsonObj)
          ...
          if (title === _self.page[i].title) {
            ...
            if (_self.page[i].type == "table") {
              ...
              _self.count2=_self.count
                            _self.name='attendence'
        var urlF = "/api/" + 'teach' + "/" + "attendenceInit";
              generalRequest(urlF, data).then((res) => {
                _self.tableList = res.data.data;
        ...
        console.log(key)
                console.log('tset')
                        console.log(key)
                console.log(value)
            _self.attendenceList[_self.count2]=value;
                      console.log(value);
                      _self.count2++
                        console.log(_self.attendenceList);
          }
      ...
      _self.list=_self.attendenceList

    },
    attendences(title){
      this.$store.commit("setRouterName", title);
      this.$emit("title", title);
      this.attendences1(title);
    }
